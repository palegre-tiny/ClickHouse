...
CREATE MATERIALIZED VIEW mv_test (`n` String, `h` AggregateFunction(groupSortedArrayWeighted(2), Int64, Int64)) ENGINE = AggregatingMergeTree ORDER BY n AS SELECT n, groupSortedArrayWeightedState(2)(h, h) as h FROM test GROUP BY n;
...
SELECT n, groupSortedArrayWeightedMerge(2)(h) from mv_test GROUP BY n;
